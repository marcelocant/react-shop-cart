...
class FloatCart extends Component {
  ...
  addProduct = product => {
    ...
    cartProducts.forEach(cp => {
      if (cp.productID === product.productID) {
        cp.unitsInStock += product.unitsInStock;
  ...
  removeProduct = product => {
    ...
    const index = cartProducts.findIndex(p => p.productID === product.productID);
    if (index >= 0) {
  ...
  changeProductQuantity = changedProduct => {
    ...
    const product = cartProducts.find(p => p.productID === changedProduct.productID);
    product.unitsInStock = changedProduct.unitsInStock;
  ...
  render() {
    ...
    const products = cartProducts.map(p => {
      return (
        <CartProduct product={p} removeProduct={removeProduct} changeProductQuantity={changeProductQuantity} key={p.productID} />
      );
    });
